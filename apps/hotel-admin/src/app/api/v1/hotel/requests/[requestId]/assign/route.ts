...
import { assignRequestSchema } from '@/server/validation/request.schema'
import { assignRequest } from '@/server/services/requests.service'
...
export async function POST(req: NextRequest, { params }: { params: { requestId: string } }) {
  try {
    ...
    const body = assignRequestSchema.parse(await req.json())
    const request = await assignRequest(user.hotelId, params.requestId, body.assigneeId, user)
