...
import { updateHotelProfileSchema } from '@/server/validation/hotel-profile.schema'
import { getHotelProfile, updateHotelProfile } from '@/server/services/hotel-profile.service'
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const body = updateHotelProfileSchema.parse(await req.json())
    const hotel = await updateHotelProfile(user.hotelId, body, user)
    return NextResponse.json(hotel)
  } catch (error) {
    return toErrorResponse(error)
  }
