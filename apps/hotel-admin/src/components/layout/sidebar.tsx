...
export function Sidebar() {
  ...
  const items = isDepartmentManager
    ? MANAGER_NAV_ITEMS
    : isDepartmentStaff
      ? STAFF_NAV_ITEMS
      : isReception
  ...
  return (
    <aside className="hidden w-60 shrink-0 border-r border-slate-200 bg-white md:flex md:flex-col">
      <div className="flex h-16 items-center gap-2.5 border-b border-slate-100 px-5">
        ...
        <span className="text-sm font-semibold text-slate-900">RoomLink Hotel Admin</span>
      </div>

      <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 py-4">
        {items.map(({ href, label, icon: Icon, group }: { href: string; label: string; icon: typeof LayoutDashboard; group?: string[] }) => {
