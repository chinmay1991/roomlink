 ...
 * the write it's logging — so audit_logs stays complete. Pass the
 * transaction client (`tx`) when called from inside
 * `prisma.$transaction(async (tx) => ...)`. Mirrors
 * apps/super-admin/src/server/audit.ts (same audit_logs table, shared DB).
 */
